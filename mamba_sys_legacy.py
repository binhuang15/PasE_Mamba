...
from edl_utils import zero_shot_easf
...
class SS2D(nn.Module):
    ...
    def _anisotropic_fuse(
        ...
        """Four-way scan fusion; with ``fusion_uncertainty`` (DPE u, [B,1,H,W]) apply ``zero_shot_easf`` (EASF: Evidence-driven Anisotropic Scan Fusion), else uniform weights."""
        B, K, C, L = out_y.shape
        assert K == self.num_directions

        inv_y = torch.flip(out_y[:, 2:4], dims=[-1]).view(B, 2, C, L)
        y0 = out_y[:, 0].view(B, C, H, W)
        y1 = torch.transpose(out_y[:, 1].view(B, C, W, H), dim0=2, dim1=3).contiguous()
        y2 = inv_y[:, 0].view(B, C, H, W)
        y3 = inv_y[:, 1].view(B, C, W, H).contiguous()

        if (fusion_uncertainty is None) or (not self.use_anisotropic_fusion):
            return y0 + y1 + y2 + y3

        Y_dirs = torch.stack([y0, y1, y2, y3], dim=1)
        u = F.interpolate(
            fusion_uncertainty, size=(H, W), mode="bilinear", align_corners=False
        ).clamp(0.0, 1.0)
        try:
            temp = float(os.environ.get("EASF_SHARPNESS_TEMP", "1.0"))
        except ValueError:
            temp = 1.0
        temp = max(temp, 1e-3)
        return zero_shot_easf(Y_dirs, u, temperature=temp)

    def forward_corev0(self, x: torch.Tensor, fusion_uncertainty: Optional[torch.Tensor] = None):
        ...
        y = self._anisotropic_fuse(out_y, H, W, fusion_uncertainty=fusion_uncertainty)
        y = y.permute(0, 2, 3, 1).contiguous()
        y = self.out_norm(y).to(x.dtype)

        return y
    
    def forward_corev0_seq(self, x: torch.Tensor, fusion_uncertainty: Optional[torch.Tensor] = None):
        self.selective_scan = selective_scan_fn
    ...
    def forward_corev1(self, x: torch.Tensor, fusion_uncertainty: Optional[torch.Tensor] = None):
        ...
        y = self._anisotropic_fuse(out_y.float(), H, W, fusion_uncertainty=fusion_uncertainty)
        y = y.permute(0, 2, 3, 1).contiguous()
        y = self.out_norm(y).to(x.dtype)
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        fusion_uncertainty = kwargs.get("fusion_uncertainty", None)
        y = self.forward_core(x, fusion_uncertainty=fusion_uncertainty)
        y = y * F.silu(z)
        out = self.out_proj(y)
...
class VSSBlock(nn.Module):
    ...
    def forward(
        self,
        input: torch.Tensor,
        runtime_attn: torch.Tensor = None,
        fusion_uncertainty: Optional[torch.Tensor] = None,
    ):
        x = input + self.drop_path(
            self.self_attention(
                self.ln_1(input),
                runtime_attn=runtime_attn,
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=1, num_classes=4, depths=[2, 2, 9, 2], 
        ...
        super().__init__()
        self._runtime_attn = None
        self._fusion_u: Optional[torch.Tensor] = None
        self._route_weights_acc: list = []
    ...
    def clear_route_weights(self) -> None:
        self._route_weights_acc.clear()

    @torch.no_grad()
    def set_runtime_attn(self, u_map: torch.Tensor, eps: float = 1e-8, renorm: bool = True):
        """
        u_map: [B,1,H,W] DPE (Decoupled Prediction-Evidence) uncertainty (higher → stronger EASF gating).
        renorm=True: per-image min–max (legacy); False: u already in [0,1] (e.g. ``dpe_easf_routing_uncertainty``).
        """
        u = u_map.clone()
        if renorm:
            u = (u - u.amin(dim=(-2, -1), keepdim=True)) / (
                u.amax(dim=(-2, -1), keepdim=True) - u.amin(dim=(-2, -1), keepdim=True) + eps
            )
        else:
            u = u.clamp(0.0, 1.0)
        self._fusion_u = u.detach()
        self._runtime_attn = (1.0 - u).detach()
        # Whether encoder stages also apply four-way uncertainty fusion (decoder always can)
        self._easf_fuse_in_encoder = os.environ.get("EASF_FUSION_ENCODER", "0").lower() in (
            "1",
            "true",
            "yes",
        )

    def _apply_runtime_attn(self, x: torch.Tensor) -> torch.Tensor:
        if self._runtime_attn is None:
            return x
        B, H, W, C = x.shape
        a = F.interpolate(self._runtime_attn, size=(H, W), mode="bilinear", align_corners=False)
        a = a.permute(0, 2, 3, 1)
        # Soft gating: avoid fully suppressing low-confidence regions (EASF_FEATURE_GATE_STRENGTH=1 restores legacy hard gating)
        try:
            g = float(os.environ.get("EASF_FEATURE_GATE_STRENGTH", "0.22"))
        except ValueError:
            g = 0.22
        g = max(0.0, min(1.0, g))
        if g <= 0.0:
    ...
    def forward_features(self, x):
        ...
        x_downsample = []
        fuse_enc = self._fusion_u if getattr(self, "_easf_fuse_in_encoder", True) else None
        for layer in self.layers:
            x_downsample.append(x)
            x = self._apply_runtime_attn(x)
            x = layer(
                x,
                runtime_attn=self._runtime_attn,
                fusion_uncertainty=fuse_enc,
            )
    ...
    def forward_up_features(self, x, x_downsample):
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                x = self._apply_runtime_attn(x)
                # First decoder stage is PatchExpand only (no SS2D) — no runtime_attn
                x = layer_up(x)
            else:
                ...
                x = layer_up(
                    x,
                    runtime_attn=self._runtime_attn,
                    fusion_uncertainty=self._fusion_u,
                )
