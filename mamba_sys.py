...
class PatchExpand(nn.Module):
    ...
    def forward(self, x, runtime_attn: torch.Tensor = None):
        x = self.expand(x)
        B, H, W, C = x.shape
...
class SS2D(nn.Module):
    ...
    def _anisotropic_fuse(
        ...
        y3 = torch.transpose(inv_y[:, 1].view(B, C, W, H), dim0=2, dim1=3).contiguous()
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=1, num_classes=4, depths=[2, 2, 9, 2], 
        ...
        self._runtime_attn = None  # set via set_runtime_attn(u_map)
    ...
    def _apply_runtime_attn(self, x):
        """
        x: [B,H,W,C] BHWC; returns gated x. No-op if attention not set.
        """
        if self._runtime_attn is None:
            return x
        B,H,W,C = x.shape
        a = F.interpolate(self._runtime_attn, size=(H,W), mode='bilinear', align_corners=False)  # [B,1,H,W]
        a = a.permute(0,2,3,1)  # [B,H,W,1]
        try:
